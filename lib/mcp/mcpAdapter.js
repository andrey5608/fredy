...
const SUPPORTED_MCP_IMAGE_MIME_TYPES = new Set(['image/jpeg', 'image/png', 'image/gif', 'image/webp']);

/**
 * Fetch one image and decode it into an MCP image content block, sniffing the format from its
 * magic bytes when the server's Content-Type header does not already name a vision-safe one.
 *
 * @param {string} imageUrl
 * @returns {Promise<{content: {type: 'image', data: string, mimeType: string}}|{error: string}>}
 */
async function fetchImageForMcp(imageUrl) {
  ...
  if (!isPubliclyFetchableUrl(imageUrl)) {
    return {
      error: `Refusing to fetch this image: its URL does not point at a public http(s) address. Image URL: ${imageUrl}`,
    };
  }
  ...
  if (!response.ok) {
    return { error: `Image fetch returned HTTP ${response.status}. Image URL: ${imageUrl}` };
  }
  ...
  try {
    ...
    return { error: `Failed to read image body: ${readErr.message}` };
  }

  const bytes = new Uint8Array(buffer);

  if (bytes.length < 12) {
    return { error: `Downloaded file is too small to determine image type. Image URL: ${imageUrl}` };
  }

  let resolvedMime;

  if (SUPPORTED_MCP_IMAGE_MIME_TYPES.has(headerMimeType)) {
    ...
    return {
      error: `Image format not supported by vision models (header: ${headerMimeType || 'unknown'}). Image URL: ${imageUrl}`,
    };
  }

  return { content: { type: 'image', data: Buffer.from(buffer).toString('base64'), mimeType: resolvedMime } };
}
...
export function createMcpServer() {
  const server = new McpServer(
    ...
    },
    {
      ...
      instructions:
        ...
        'get_photo_for_listing or get_photos_for_listing (single photo vs. the full stored gallery) for vision analysis, ' +
  ...
  server.tool(
    ...
    async ({ listingId }, extra) => {
      ...
      const result = await fetchImageForMcp(imageUrl);
      if (result.error) {
        return normalizeError(result.error, 'get_photo_for_listing');
      }
  ...
  server.tool(
    'get_photos_for_listing',
    'Fetch and return every stored photo of a listing (up to the admin-configured gallery limit) as images for vision analysis. ' +
      'Use this instead of get_photo_for_listing when more than one angle of the property matters.',
    {
      listingId: z.string().describe('The listing ID whose photos to fetch'),
    },
    { title: 'Get listing photos', readOnlyHint: true, idempotentHint: true, openWorldHint: true },
    async ({ listingId }, extra) => {
      const { user, error } = authenticateToolCall(extra, 'get_photos_for_listing');
      if (error) return normalizeError(error, 'get_photos_for_listing');

      const listing = getListingById(listingId, user.id, user.isAdmin);
      if (!listing) {
        return normalizeError('Listing not found or access denied.', 'get_photos_for_listing');
      }

      // `getListingById` already attaches the stored gallery (or falls back to the single
      // image_url), and storage already caps it at the admin-configured maxImagesPerListing - no
      // extra limit is applied here.
      const imageUrls = Array.isArray(listing.images) && listing.images.length > 0 ? listing.images : [];
      if (imageUrls.length === 0) {
        return normalizeError('No images available for this listing.', 'get_photos_for_listing');
      }

      const results = await Promise.all(imageUrls.map((url) => fetchImageForMcp(url)));
      const content = results.filter((r) => r.content).map((r) => r.content);
      const failedCount = results.length - content.length;

      if (content.length === 0) {
        return normalizeError(
          `Failed to fetch any of the ${imageUrls.length} image(s) for this listing.`,
          'get_photos_for_listing',
        );
      }
      ...
      return {
        content:
          failedCount > 0
            ? [
                ...content,
                { type: 'text', text: `Note: ${failedCount} of ${imageUrls.length} image(s) could not be fetched.` },
              ]
            : content,
      };
    },

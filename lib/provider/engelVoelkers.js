...
/**
 * Images are referenced by their uploadcare id only, so the delivery url has to be assembled for
 * each one. `uploadCareImages` (objects carrying an `id`) is preferred; `uploadCareImageIds` (bare
 * ids) is the fallback for a listing that only ever had the older shape.
 *
 * @param {any} listing the raw listing
 * @returns {string[]} every image url the listing has, in gallery order
 */
function buildImageUrls(listing) {
  const ids =
    Array.isArray(listing?.uploadCareImages) && listing.uploadCareImages.length > 0
      ? listing.uploadCareImages.map((image) => image?.id)
      : (listing?.uploadCareImageIds ?? []);
  return ids.filter((id) => id != null).map((id) => `${UPLOADCARE_URL}/${id}/-/format/jpeg/-/resize/1080x/`);
}
...
function buildImageUrl(listing) {
  return buildImageUrls(listing)[0] ?? null;
}

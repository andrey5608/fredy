...
function normalize(o) {
  ...
  return {
    ...
    // The search card already carries the full gallery - `image` above is just its first entry.
    // Nothing here costs an extra request; it is the same `o.gallery.images` array being read
    // twice, once for the one field every provider has and once for the ones only some do.
    images: (o?.gallery?.images ?? []).map((entry) => entry?.url).filter((url) => typeof url === 'string'),

...
async function pushDetails(listing) {
  ...
  // `image` (the search card's titlePicture, set in normalize()) is left as-is rather than
  // reassigned to images[0]: they are usually the same photo but come from two different API
  // responses, and normalize() already ran before this ever gets called.
  listing.images = readImages(detailBody);
...
/**
 * Every gallery image of the exposé, largest size first.
 *
 * The section is found by its `type` rather than assumed to be `sections[0]`: `MEDIA` is what the
 * mobile API actually calls it, and the exposé is laid out per listing (a plot with no photos at
 * all has been seen to open straight on `TOP_ATTRIBUTES`), so indexing in would occasionally read
 * the wrong section as an empty gallery instead of the right one.
 *
 * @param {any} detailBody Parsed `/expose/{id}` response.
 * @returns {string[]}
 */
function readImages(detailBody) {
  const media = (detailBody?.sections || []).find((section) => section?.type === 'MEDIA')?.media;
  if (!Array.isArray(media)) return [];
  return media.map((entry) => entry?.fullImageUrl ?? entry?.imageUrlForWeb).filter((url) => typeof url === 'string');
}

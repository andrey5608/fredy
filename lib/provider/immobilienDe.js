...
function normalize(o) {
  ...
  return {
    ...
    // The search card already carries every image, not just the one card thumbnail - same field,
    // read in full instead of at index 0, no extra request.
    images: Array.isArray(o.images) ? o.images.map((url) => plain(url)).filter((url) => typeof url === 'string') : [],

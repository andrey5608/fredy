...
function extractDetailFromHtml(html) {
  ...
  return {
    ...
    detailImages: extractImagesFromHtml($),
  };
}

/**
 * Every gallery image of the detail page, in display order.
 *
 * Scoped to the main viewer (`.j-gallery-image .galleryimage-element`) rather than to
 * `img[data-imgsrc]` anywhere on the page: the lightbox's own thumbnail strip
 * (`#viewad-lightbox-thumbnail-list`) repeats the same photos at a different resolution, and the
 * "other ads by this seller" section further down the page carries unrelated listings' images with
 * no `data-imgsrc` at all - but relying on that alone to tell them apart would be fragile.
 *
 * `data-imgsrc` is preferred over `src` because it is the full-size delivery URL; `src` on this
 * element is a low-resolution placeholder swapped in by the page's own lazy-loading.
 *
...
function extractImagesFromHtml($) {
  const urls = [];
  $('.j-gallery-image .galleryimage-element img').each((_, element) => {
    const node = $(element);
    const url = node.attr('data-imgsrc') || node.attr('src');
...
async function enrichListingFromDetails(listing, browser) {
  ...
  try {
    ...
    const { detailAddress, detailDescription, detailSize, detailRooms, detailBuildYear, detailImages } =
    ...
    return {
      ...
      // The search card only ever has the one thumbnail (crawlFields.image, set in normalize());
      // the full gallery only exists on this page, which fetchDetails already has to load for the
      // fields above.
      images: detailImages.length > 0 ? detailImages : listing.image ? [listing.image] : [],
    };
  } catch (error) {

...
const mapListings = (serviceName, jobKey, listings, baseUrl) =>
  listings.map((l) => {
    ...
    // back to that single field for every provider outside the multi-image set. Unlike smtp/resend,
    // SendGrid renders its own operator-authored template (templateId) rather than ours, so
    // `images` is exposed as data for that template to loop over if the operator chooses to -
    // there is no markup here to update.
    const images = normalizeImageUrls(l.images?.length > 0 ? l.images : [l.image]);
    return {
      title: l.title || '',
      link: l.link || '',
      address: l.address || '',

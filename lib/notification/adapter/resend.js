...
const mapListings = (serviceName, jobKey, listings, baseUrl) =>
  listings.map((l) => {
    ...
    // back to that single field for every provider outside the multi-image set.
    ...
    return {
      ...
      commute: l.commute || '',
      image: images[0] ?? null,
      // The template shows the first as the hero image and the rest, if any, as a thumbnail row -
      // splitting here rather than in the template keeps Handlebars free of array-slicing logic.

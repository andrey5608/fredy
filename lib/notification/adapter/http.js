...
const mapListing = (listing, baseUrl) => ({
  ...
  // Full gallery alongside the pre-existing single `imageUrl`, so a receiver built before this
  // existed keeps working unchanged while a new one can read every photo.
  imageUrls: normalizeImageUrls(listing.images?.length > 0 ? listing.images : [listing.image]),
  price: listing.price,
  size: listing.size,
  title: listing.title,

...
import { DEFAULT_MAX_IMAGES_PER_LISTING } from './migrations/sql/46.listing-images.js';
...
export const getProviderDistributionForJobIds = (jobIds = []) => {
  ...
  const placeholders = jobIds.map(() => '?').join(',');
  // Use configured providers from jobs to ensure multi-provider jobs are counted even without listings
  const rows = SqliteConnection.query(
    `SELECT json_extract(p.value, '$.id') AS providerId, COUNT(*) AS cnt
     FROM jobs j
            JOIN json_each(j.provider) AS p
 ...
 * @param {Object} [options]
 * @param {number} [options.maxImagesPerListing] - Cap on how many `listing_images` rows one listing
 *   gets. Defaults to {@link DEFAULT_MAX_IMAGES_PER_LISTING} so callers that do not thread the
 *   configured value through (every existing test, for instance) still behave sensibly.
 * @returns {void}
 */
export const storeListings = (
  ...
  providerId,
  listings,
  { maxImagesPerListing = DEFAULT_MAX_IMAGES_PER_LISTING } = {},
) => {
  ...
  SqliteConnection.withTransaction((db) => {
    ...
    const insertImage = db.prepare(
      `INSERT INTO listing_images (id, listing_id, url, position, created_at) VALUES (@id, @listing_id, @url, @position, @created_at)`,
    );
    ...
    for (const item of listings) {
      ...
      // `RETURNING id` now answers on a conflict too (see the comment on `stmt` above), so
      // `inserted != null` alone no longer tells a real insert apart from an update - both return a
      // row. The `id` column is absent from the UPDATE SET clause, so a conflict's row keeps its
      // original id, never `params.id` (freshly generated for this attempt); a real insert's
      // returned id is always exactly `params.id`.
      const isNewRow = inserted?.id === params.id;

      // Only for a listing actually written this run - a conflict means the row (and whatever
      // images it already has) predates this batch, and re-inserting would duplicate them.
      if (isNewRow && Array.isArray(item.images) && item.images.length > 0) {
        const urls = [...new Set(item.images.filter((url) => typeof url === 'string' && url.length > 0))].slice(
          0,
          maxImagesPerListing,
        );
        urls.forEach((url, position) => {
          insertImage.run({ id: nanoid(), listing_id: item.id, url, position, created_at: params.created_at });
        });
      }
...
/**
 * Every stored image url for a set of listings, keyed by listing id and ordered by position.
 *
 * One stored image row by its own id, for the image proxy (`imagesRouter.js`) to resolve the
 * origin url and the listing it belongs to before deciding whether the requester may see it.
 *
 * @param {string} imageId
 * @returns {{id: string, listing_id: string, url: string}|null}
 */
export const getImageById = (imageId) => {
  if (!imageId) return null;
  return (
    SqliteConnection.query(`SELECT id, listing_id, url FROM listing_images WHERE id = @imageId`, {
      imageId,
    })[0] ?? null
  );
};
...
/**
 * Chunked the same way {@link getTravelTimesForListings} is: an unbounded id list would otherwise
 * risk SQLite's bound-parameter ceiling.
 *
 * @param {string[]} ids
 * @returns {Map<string, string[]>}
 */
export const getImagesForListings = (ids) => {
  ...
  for (let offset = 0; offset < unique.length; offset += ID_CHUNK_SIZE) {
    ...
    for (const row of rows) {
      ...
      if (existing) {
        ...
        byListing.set(row.listing_id, [row.url]);
      }
    }
  }
  return byListing;
...
/**
 * Attach every stored image to listing rows, in place.
 *
 * Only ever called for a handful of rows at a time (one listing's detail page, one job run's worth
 * of new listings) - never for a paginated page of the overview, which keeps reading its thumbnail
 * straight off `image_url` the way it always has. Joining the gallery into every row of a
 * fifty-listing page would be paying for it fifty times over for a column almost nothing there
 * reads.
 *
 * A row whose listing has no `listing_images` rows at all (every provider outside the MVP set, or
 * one stored before this feature existed) falls back to its own `image_url` - the one-image
 * experience nothing regresses out of.
 *
 * @template {{id: string, image_url?: string|null}} T
 * @param {T[]} rows
 * @returns {T[]} The same rows.
 */
export const attachImages = (rows) => {
  if (!Array.isArray(rows) || rows.length === 0) return rows;
  const byListing = getImagesForListings(rows.map((row) => row.id));
  for (const row of rows) {
    const stored = byListing.get(row.id);
    row.images = stored != null && stored.length > 0 ? stored : row.image_url ? [row.image_url] : [];
  }
  return rows;
};
...
export const getListingById = (id, userId = null, isAdmin = false) => {
  ...
  return attachTravelTimes([row], { includeGeometry: true })[0];
};

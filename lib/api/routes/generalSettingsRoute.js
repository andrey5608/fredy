...
/**
 * Bounds for the listing image settings.
 *
 * `maxImagesPerListing` caps both how many `listing_images` rows a listing gets and, downstream,
 * how many a notification adapter sends - one provider with an unusually large gallery must not be
 * able to balloon the database or flood a channel. `imageCacheRetentionDays` bounds how long a
 * downloaded-and-compressed copy is kept on disk before the cleanup cron reclaims it.
 * @type {Record<string, {min: number, max: number}>}
 */
const IMAGE_SETTING_BOUNDS = {
  maxImagesPerListing: { min: 0, max: 50 },
  imageCacheRetentionDays: { min: 0, max: 365 },
};

/**
 * Validate one numeric listing-image setting.
 *
 * @param {string} name The setting name, used in the message.
 * @param {any} value The raw value from the request body.
 * @returns {string|null} An error message, or null when the value is acceptable.
 */
function validateImageSetting(name, value) {
  const bounds = IMAGE_SETTING_BOUNDS[name];
  if (value === '' || value === null) {
    return `${name} must be a number.`;
  }
  const parsed = Number(value);
...
export default async function generalSettingsPlugin(fastify) {
  ...
  fastify.post('/', async (request, reply) => {
    ...
    for (const name of Object.keys(IMAGE_SETTING_BOUNDS)) {
      if (typeof appSettings[name] === 'undefined') continue;
      const error = validateImageSetting(name, appSettings[name]);
      if (error != null) {
        return reply.code(400).send({ error });
      }
      appSettings[name] = Number(appSettings[name]);

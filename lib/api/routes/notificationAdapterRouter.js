...
export default async function notificationAdapterPlugin(fastify) {
  ...
  fastify.get('/existing', async (request, reply) => {
    const currentUser = request.session?.currentUser;
    const canSeeAll = isAdmin(request);

    try {
      const jobs = jobStorage.getJobs();
      return collectExistingAdapters({ jobs, currentUser, isAdmin: canSeeAll });
    } catch (error) {
      logger.error('Error while fetching existing notification adapters:', error);
      return reply.code(500).send({ error: 'Error while fetching existing notification adapters.' });
    }
  });

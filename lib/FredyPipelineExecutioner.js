...
import { DEFAULT_MAX_IMAGES_PER_LISTING } from './services/storage/migrations/sql/42.listing-images.js';
...
class FredyPipelineExecutioner {
   ...
   */
  async _save(newListings) {
    logger.debug(`Storing ${newListings.length} new listings (Provider: '${this._providerId}')`);
    const settings = await getSettings();
    const maxImagesPerListing = settings?.maxImagesPerListing ?? DEFAULT_MAX_IMAGES_PER_LISTING;
    storeListings(this._jobKey, this._providerId, newListings, { maxImagesPerListing });
    return newListings;
  }
